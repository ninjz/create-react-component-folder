...
function createFiles(componentName, componentPath, cssFileExt) {
  ...
  return new Promise((resolve) => {
    // Custom
    const isCustom = config && config.includes('custom');
        ...
        for (let i = 0; i < files.length; i += 1) {
          ...
          if (file === indexFile) {
            ...
            if (functional || (config && config.includes('functional'))) {
              ...
              data = isNative
                ? isCustom ? createTypeScriptReactNativeFunctionalComponent(name) : createTypeScriptReactNativeComponent(name)
          ...
          if ((file.indexOf(`.stories.${ext}`) || file.indexOf(`.stories.${ext}`)) > -1) {
            data = isCustom ? createReactNativeComponentStories(
              name,
              uppercase || (config && config.includes('uppercase')),
              isTypeScript,
            ) : createReactComponentStories(
